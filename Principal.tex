\documentclass[a4paper,12pt,oneside,openany]{book}
\input{TesePack}
\usepackage[hidelinks]{hyperref}
\usepackage[bottom]{footmisc}
\usepackage{array}
\usepackage{graphicx}
\usepackage{longtable}
\usepackage{listings}
\usepackage[normalem]{ulem} % Strikethrough
\usepackage{pgfplots} % Plot
\pgfplotsset{holdot/.style={color=blue,fill=white,only marks,mark=*}} % Plot
\usepackage{placeins} % To use \FloatBarrier and keep image inside section
\usepackage[page,toc,titletoc,title]{appendix} % Appendix
% \usepackage{pythonhighlight} % Python highlighting
% \usepackage{minted} % Python highlighting
\usepackage{lscape} % Landscape page for database ERD Image
\usepackage{arydshln} % For horizontal dashed line in tables (\hdashline)
\usepackage{notoccite} % Ensure citation sequence
\usepackage{floatrow} % Ensure table captions on top
\floatsetup[table]{capposition=top} % Ensure table captions on top
\usepackage{floatrow} % Ensure figure captions on top
\floatsetup[figure]{capposition=top} % Ensure figure captions on top
\usepackage{url} % Ensure bibliography urls break lines
% \setcounter{biburllcpenalty}{7000} % Ensure bibliography urls break lines
% \setcounter{biburlucpenalty}{8000} % Ensure bibliography urls break lines

\usepackage{fifo-stack}
\FSCreate{colors}{black}
\makeatletter
\let\old@color\color
\renewcommand\color[1]{\FSPush{colors}{#1}\old@color{#1}}
\newcommand\colorend{\FSPop{colors}\old@color{\FSTop{colors}}}

\newcommand{\titulo}{TÉCNICAS DE APRENDIZADO DE MÁQUINA APLICADAS A ESTRATÉGIA DE SWING TRADE NO MERCADO FINANCEIRO}

\begin{document}

\frontmatter
\thispagestyle{empty}

\include{Capa}

\include{Preambulo}

% Table of Contents
% ---------------------------------------------------------------
\tableofcontents
% ---------------------------------------------------------------
% Lista de figuras
% ---------------------------------------------------------------
%\cleardoublepage
%\addcontentsline{toc}{chapter}{Lista de Figuras}
\listoffigures
% ---------------------------------------------------------------
% Lista de Tabelas
% ---------------------------------------------------------------
%\cleardoublepage
%\addcontentsline{toc}{chapter}{Lista de Tabelas}
\listoftables

\mainmatter
\cleardoublepage


\include{cap1}
\include{cap2}
\include{cap3}
\include{cap4}
\include{cap5}

% ---------------------------------------------------------------
% Bibliografia
% ---------------------------------------------------------------
\normalsize
\cleardoublepage
\addcontentsline{toc}{chapter}{Bibliografia}
\bibliographystyle{coppe}
\bibliography{biblio}


% Apendices

\appendix

% Apendice A


\include{ApendiceA}

% % Apendice B

% \chapter{Encadernação do Projeto de Graduação}
% \label{ApendiceB}
% \include{ApendiceB}

% % Apendice C

% \chapter{O que é um anexo}
% \label{ApendiceC}
% \include{ApendiceC}

\backmatter

\end{document}